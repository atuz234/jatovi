...
class quanlysanpham_model extends JATOVI_Model
{
	...
	{
		extract($mang);
		$sql = "UPDATE {$this->_table} SET ten ='{$ten}', mota = '{$mota}', giacu = '{$giacu}', giamoi = '{$giamoi}', ngaysanxuat = '{$ngaysanxuat}', hansudung = '{$hansudung}', donvi = '{$donvi}', hinhanh = '{$hinhanh}' WHERE id = '{$id}'";
		$query = $this->connection->prepare($sql);
		$query->execute();
	}
